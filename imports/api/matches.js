Template.matchList.helpers({
	WCMatches: function(){
		return matchesdb.find({}, {sort: {matchNum: 1}, limit: (Number(VFWCdb.findOne({}).matchesAvailable)-(goalsdb.find().count()/2)), skip: (goalsdb.find().count()/2)});
...
Template.setGoals2.events({
	'click .js-setGoals': function(){
		var mId = $('#setGoalsModal2 input[name="matchID"]').val();
		var tg1 = Number($('#teamGoals12').val());
		var tg2 = Number($('#teamGoals22').val());
		var teamData = matchesdb.findOne({_id: mId});		
		var result = goalsdb.find({'matchID': mId});
		result.forEach(function(goalDocs){				
			if (goalDocs.teamID == teamData.teamID1){
